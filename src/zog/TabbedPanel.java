...
import utils.CloseTabButton;
import utils.filefilter.BmpFilter;
import utils.filefilter.ImageFilter;
import utils.filefilter.JpegFilter;
import utils.SavePopup;
...
public final class TabbedPanel extends JPanel implements UtilData {
	...
	public void save() {
		...
		File file = null;
		if (data.get(tabPanel.getSelectedIndex()) instanceof String) {
			saveAs();
		} else {
			file = (File) data.get(tabPanel.getSelectedIndex());
			BufferedImage img = ((DrawPanel) ((JScrollPane) tabPanel
	...
	public void saveAs() {
		...
		if (returnVal == JFileChooser.APPROVE_OPTION) {
			...
			if (file.getName().endsWith(".jpg")) {
				...
				// this.setTitle(imFile.getName());
				try {
					ImageIO.write(image, "bmp", file);
				} catch (IOException e) {
					JOptionPane.showMessageDialog(this, "Write error for \n"
				...
				try {
					String ext = "bmp";
					File f = new File(file.getCanonicalPath() + "." + ext);
					if (fC.getFileFilter() instanceof JpegFilter) {
						ext = "jpg";
						new SavePopup(image, f);
					} else {
